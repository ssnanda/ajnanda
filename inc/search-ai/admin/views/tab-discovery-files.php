...
<div class="ajnanda-admin-section">
    ...
    <div class="ajnanda-admin-grid ajnanda-discovery-grid">
        ...
        <div class="ajnanda-admin-card"><h2><?php esc_html_e('robots.txt', 'ajnanda'); ?></h2><span class="ajnanda-admin-pill is-success"><?php esc_html_e('WordPress policy available', 'ajnanda'); ?></span><?php if ($robots_endpoint && $robots_endpoint['reachable']) : ?><span class="ajnanda-admin-pill is-success"><?php esc_html_e('Public endpoint reachable', 'ajnanda'); ?></span><?php elseif ($robots_endpoint) : ?><span class="ajnanda-admin-pill is-warning"><?php echo $robots_endpoint['code'] ? esc_html(sprintf(__('Public endpoint returned HTTP %d', 'ajnanda'), $robots_endpoint['code'])) : esc_html__('Public endpoint could not be reached', 'ajnanda'); ?></span><?php endif; ?><p><?php esc_html_e('Preserves WordPress rules and appends AJNanda’s registry-backed AI crawler policy. A public endpoint warning usually indicates web-server or reverse-proxy routing, not a broken WordPress policy.', 'ajnanda'); ?></p><a class="button" target="_blank" rel="noopener" href="<?php echo esc_url($discovery_status['robots']['url']); ?>"><?php esc_html_e('View robots.txt', 'ajnanda'); ?></a></div>
        <div class="ajnanda-admin-card"><h2><?php esc_html_e('llms.txt', 'ajnanda'); ?></h2><span class="ajnanda-admin-pill <?php echo $discovery_status['llms_txt']['enabled'] ? 'is-success' : 'is-warning'; ?>"><?php echo $discovery_status['llms_txt']['enabled'] ? esc_html__('Enabled', 'ajnanda') : esc_html__('Disabled or delegated', 'ajnanda'); ?></span><?php if ($llms_endpoint) : ?><span class="ajnanda-admin-pill <?php echo $llms_endpoint['reachable'] ? 'is-success' : 'is-warning'; ?>"><?php echo $llms_endpoint['reachable'] ? esc_html__('Public endpoint reachable', 'ajnanda') : esc_html(sprintf(__('Endpoint returned %s', 'ajnanda'), $llms_endpoint['code'] ? 'HTTP ' . $llms_endpoint['code'] : __('no response', 'ajnanda'))); ?></span><?php endif; ?><p><?php printf(esc_html__('Owner: %s. Uses the Site Profile and excludes content blocked from llms.txt advertising.', 'ajnanda'), esc_html($owner_label($discovery_status['llms_txt']))); ?></p><?php if ($discovery_status['llms_txt']['enabled']) : ?><a class="button" target="_blank" rel="noopener" href="<?php echo esc_url($discovery_status['llms_txt']['url']); ?>"><?php esc_html_e('View llms.txt', 'ajnanda'); ?></a><?php endif; ?></div>
