...
<div class="ajnanda-admin-card">
    ...
    <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
        ...
        <table class="form-table" role="presentation">
            ...
            <tr><th scope="row"><label for="seo_default_social_image"><?php esc_html_e('Default Social Share Image', 'ajnanda'); ?></label></th><td><input type="text" id="seo_default_social_image" name="seo_default_social_image" value="<?php echo esc_attr($values['seo_default_social_image']); ?>" class="regular-text"> <button type="button" class="button" id="ajnanda_seo_social_image_button"><?php esc_html_e('Choose Image', 'ajnanda'); ?></button><p class="description"><?php esc_html_e('Same-site images are saved as root-relative paths so environment syncs remain portable.', 'ajnanda'); ?></p></td></tr>
            ...
            <tr><th scope="row"><label for="seo_analytics_measurement_id"><?php esc_html_e('Analytics Measurement ID', 'ajnanda'); ?></label></th><td><input type="text" id="seo_analytics_measurement_id" name="seo_analytics_measurement_id" value="<?php echo esc_attr($values['seo_analytics_measurement_id']); ?>" class="regular-text" placeholder="G-XXXXXXXXXX"><p class="description"><?php esc_html_e('Google Analytics 4 (G-), Google tag (GT-), Google Ads (AW-) or Campaign Manager (DC-). AJNanda emits the standard gtag.js snippet. A consent plugin can defer it via the ajnanda_analytics_should_load filter.', 'ajnanda'); ?></p></td></tr>
            <tr><th scope="row"><?php esc_html_e('Existing output', 'ajnanda'); ?></th><td><label><input type="checkbox" name="seo_schema_enabled" value="1" <?php checked($values['seo_schema_enabled']); ?>> <?php esc_html_e('Enable Schema Markup', 'ajnanda'); ?></label><br><label><input type="checkbox" name="seo_llms_txt_enabled" value="1" <?php checked($values['seo_llms_txt_enabled']); ?>> <?php esc_html_e('Publish /llms.txt and /llms-full.txt', 'ajnanda'); ?></label><p class="description"><?php esc_html_e('AI crawler access is managed separately on the AI Discovery tab.', 'ajnanda'); ?></p></td></tr>
        </table>
