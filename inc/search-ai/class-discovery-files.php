...
class AJNanda_Search_AI_Discovery_Files {
    ...
    public static function endpoint_status($url, $cache_suffix = '') {
        $compare_llms = 'llms' === $cache_suffix;
        $expected = $compare_llms ? self::render_llms_txt() : '';
        $cache_key = 'ajnanda_search_ai_endpoint_v3_' . md5($cache_suffix . '|' . $url . '|' . hash('sha256', $expected) . '|' . (int) self::llms_enabled());
        $cached = get_transient($cache_key);
        if (is_array($cached)) { return $cached; }
        $response = wp_safe_remote_get($url, array(
            'timeout' => 3, 'redirection' => 1, 'limit_response_size' => $compare_llms ? 1048577 : 2048,
        ...
        if (is_wp_error($response)) {
            ...
            if ($compare_llms && 200 === $code) {
                $body = wp_remote_retrieve_body($response);
                $status['content_match'] = strlen($body) > 1048576 ? null : hash_equals(hash('sha256', $expected), hash('sha256', $body));
                $status['hostinger_signature'] = false !== strpos($body, '[comment]: # (Generated by Hostinger Tools Plugin)');
            }
        }
    ...
    public static function physical_llms_status() {
        require_once ABSPATH . 'wp-admin/includes/file.php';
        $paths = array_unique(array(trailingslashit(get_home_path()) . 'llms.txt', ABSPATH . 'llms.txt'));
        $expected_hash = hash('sha256', self::render_llms_txt());
        $files = array();
        foreach ($paths as $path) {
            if (! file_exists($path)) { continue; }
            $body = is_file($path) && is_readable($path) ? file_get_contents($path, false, null, 0, 1048577) : false;
            $files[] = array(
                'path' => $path,
                'match' => false === $body || strlen($body) > 1048576 ? null : hash_equals($expected_hash, hash('sha256', $body)),
                'hostinger_signature' => false !== $body && false !== strpos($body, '[comment]: # (Generated by Hostinger Tools Plugin)'),
            );
        }
        return $files;
    }
