...
| **Search & AI** | Theme-native discovery workflow: canonical Site Profile, central Content Access Policy, SEO/plugin capability ownership, schema, crawler-category robots policy, `/llms.txt` with curated Important Pages plus 16 dynamically selected recent articles, eligible full content in `/llms-full.txt`, editable discovery outputs, optional Hostinger/Web2Agent status with native consent navigation and llms.txt conflict diagnostics, WordPress sitemap filtering, objective readiness checks, cached Site Kit insights, and a bounded WordPress-local Crawler Log with reported-vs-verified identity, privacy modes, retention, filters, pagination, and future external-source support | `inc/search-ai/`, `inc/seo.php`, `docs/search-ai.md` |
